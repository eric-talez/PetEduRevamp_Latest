...
- **반려견 예방접종 QR 여권 (Task #148, May 10, 2026)**: 보호자가 강아지 등록번호+예방접종 기록을 등록하면 보호자 페이지(`/my-pets`)에서 1년 유효 QR 여권 발급. QR을 카메라(`/verify/pet/scan`)로 스캔하면 공개 검증 페이지(`/verify/pet/:token`)에서 마스킹된 등록번호·반려동물 기본정보·백신별 상태(유효/30일내만료/만료/미접종) 확인 가능. Tables: `pets.registration_number(varchar 30)`, `pet_vaccination_passports`(token unique 128, expiresAt 1년, isActive, verifyCount). Routes: `POST /api/pets/:petId/passport/issue`(발급, 등록번호+백신 1건 이상 필수, 기존 활성 토큰 자동 회수), `GET /api/pets/:petId/passport`(조회+발급조건), `POST /api/pets/:petId/passport/revoke`(회수), `GET /api/pet-passport/verify/:token`(공개 검증, 30 req/min IP 제한). 발급 토큰은 `crypto.randomBytes(32).base64url`. 보호자 PII(이름/주소/연락처)는 검증 응답에서 제외하고 등록번호는 앞 4자리만 노출. 컴포넌트: `PetPassportCard`(QR 표시·복사·다운로드·재발급·회수), `pet-scan.tsx`(BarcodeDetector API + 폴백 직접 입력), `pet-result.tsx`(전체 상태 배너 + 백신 카드 리스트).
...
- **Vertex AI Search 보조 검색 (Task #231 + Task #255, May 16, 2026)**: GCP Discovery Engine을 **보조 검색(Secondary search) 전용**으로 운용. Advanced website indexing은 소유 도메인 인증이 필요하므로 외부 도메인(k-pet.co.kr, megazoo.co.kr 등)을 색인할 수 없어 자동 수집(runImport) 경로에서 **제외**됨. 주 수집은 Naver/Daum/Google 크롤러 폴백이 담당. Vertex는 `searchEventRecords(query)` 온디맨드 보조 검색과 관리자 테스트(`testVertexAiSearchOnce`)에만 사용됨. 403 오류를 두 가지로 구분: `iam_permission_denied`(서비스 계정 IAM 권한 없음) vs `unverified_indexing`(Advanced website indexing 소유권 미인증). Basic website search(소유권 인증 불필요)는 `VERTEX_AI_SEARCH_BASIC_DATASTORE` 환경 변수로 활성화 가능(저신뢰도 후보 풀 전용). Source Catalog(`SOURCE_CATALOG` 상수, server/services/eventUpdater.ts)에서 모든 수집 소스의 전략/메타/primary 여부를 선언적으로 관리. 관리자 `/admin/pet-events` 공급자 상태 카드에 3줄 경고 표시 + "보조 검색(Secondary search)" 레이블. 환경 변수: `VERTEX_AI_SEARCH_PROJECT`, `VERTEX_AI_SEARCH_DATASTORE`, `VERTEX_AI_SEARCH_LOCATION`(기본 `global`), `GOOGLE_APPLICATION_CREDENTIALS_JSON`(roles/discoveryengine.viewer), `VERTEX_AI_SEARCH_BASIC_DATASTORE`(선택, Basic website search용).
