...
- **Training Journal Photo/Video Attachments (May 03, 2026)**: Trainers can attach up to 5 images (10MB each, jpeg/png/webp/gif) and 1 video (50MB, mp4/webm/mov) per 알림장. Schema `notebookAttachments` table (journalId, kind, storageKey, thumbnailKey, sizeBytes, mimeType, sortOrder, uploadedBy) + index. ObjectStorage helpers `uploadBufferToPrivate / getPrivateFileByKey / deletePrivateByKey` save into `.private/notebook/<journalId>/{images|videos|thumbs}/<uuid>.<ext>`. Routes (gated by `requireAuth + canUserAccessTrainingJournal` for reads, `+ canUserModifyTrainingJournal + csrfProtection` for writes): `GET/POST /api/notebook/entries/:id/attachments`, `DELETE /api/notebook/attachments/:id`, `GET /api/notebook/attachments/:id` & `/thumbnail` with HTTP Range support (validated for reversed/out-of-bounds → 416). Multer memoryStorage with multer-error middleware mapping `LIMIT_FILE_SIZE`→413. Video first-frame thumbnail via fluent-ffmpeg in `try/finally` cleanup. Shared `JournalAttachmentManager` component (TanStack Query, `secureRequest`, drag/drop + file picker, thumbnail grid, lightbox with arrow-key nav, delete) embedded in trainer detail dialog (`canEdit=true`) and pet-owner detail dialog (`canEdit=false`). PDF export (`generateNotebookPdf`) now downloads first 1–2 image attachments and embeds them under "훈련 사진" section side-by-side.
