...
class Storage {
  ...
  async getInstituteIdsForTrainer(userId: number): Promise<number[]> {
    const rows = await db
      .select({ instituteId: trainerInstitutes.instituteId })
  ...
  async listNotebookTemplatesForUser(userId: number, instituteIds: number[] = []): Promise<NotebookTemplate[]> {
    const conditions = [
      eq(notebookTemplates.isSystem, true),
      eq(notebookTemplates.ownerUserId, userId),
    ];
    ...
    return rows.sort((a, b) => {
      const rank = (t: NotebookTemplate) => t.isSystem ? 0 : (t.ownerUserId === userId ? 1 : 2);
  ...
  async getNotebookTemplate(id: number): Promise<NotebookTemplate | null> {
    const rows = await db.select().from(notebookTemplates).where(eq(notebookTemplates.id, id)).limit(1);
    return rows[0] ?? null;
  }

  async createNotebookTemplate(data: { ownerUserId: number; instituteId?: number | null; name: string; body: string; category?: string | null; homeworkPreset?: any | null }): Promise<NotebookTemplate> {
  ...
  async updateNotebookTemplate(id: number, data: Partial<{ name: string; body: string; category: string | null; homeworkPreset: any | null; instituteId: number | null }>): Promise<NotebookTemplate | null> {
    const patch: Record<string, unknown> = { updatedAt: new Date() };
    if (data.name !== undefined) patch.name = data.name;
    if (data.body !== undefined) patch.body = data.body;
    if (data.category !== undefined) patch.category = data.category;
    if (data.homeworkPreset !== undefined) patch.homeworkPreset = data.homeworkPreset;
    if (data.instituteId !== undefined) patch.instituteId = data.instituteId;
    const [row] = await db.update(notebookTemplates).set(patch).where(eq(notebookTemplates.id, id)).returning();
    return row ?? null;
  }

  async deleteNotebookTemplate(id: number): Promise<boolean> {
    const result = await db.delete(notebookTemplates).where(eq(notebookTemplates.id, id)).returning({ id: notebookTemplates.id });
    return result.length > 0;
  }

  async duplicateNotebookTemplate(id: number, ownerUserId: number): Promise<NotebookTemplate | null> {
  ...
  async incrementNotebookTemplateUsage(id: number): Promise<void> {
    await db
...
class HybridStorage extends Storage {
  ...
  async bulkDeactivatePetEvents(ids: number[]): Promise<number> {
    await this.ensurePetEventsTable();
    if (!ids.length) return 0;
    const result = await db
      .update(petEvents)
